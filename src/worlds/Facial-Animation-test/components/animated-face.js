...
AFRAME.registerComponent('animated-face', {
    ...
    stateHandler: async function (state){
        ...
        switch(state){
            ...
            case 'idle':
                CONTEXT_AF.idle();
                activeState = setInterval( async => CONTEXT_AF.idle(), 10000);
    ...
    createAnimations: async function (context){
        ...
        context.el.setAttribute('animation__mouthUpperUpRight', {   property: 'gltf-morph__mouthUpperUpRight.value',
                                                                to: 0.75, loop: false, dur: 500, autoplay: false, 
    ...
    idle: async function (){
        ...
        await delay(1000);
        this.el.emit('relaxMouthPressRight');
        this.el.emit('moveEyes', {value: 'look', side: 'bothRight', dur: 6});
        await delay(2000);
        this.el.emit('moveEyes', {value: 'blink', side: 'both', dur: 1});
        await delay(500);
        this.el.emit('relaxMouthPress');
        await delay(500);
        this.el.emit('moveEyes', {value: 'look', side: 'bothLeft', dur: 6});
        await delay(2000);
