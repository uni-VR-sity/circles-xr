...
AFRAME.registerComponent('queue-circles-ai-agent', 
{
    schema: 
    {
    ...
    getResponse: async function()
    {
        ...
        if (this.currentInput)
        {
            ...
            try 
            {
                ...
                // Splitting response into sentences (reverse to use pop())
                this.currentInput.sentences = this.currentInput.response.split(/(?<=[.!?])\s+/).reverse();
